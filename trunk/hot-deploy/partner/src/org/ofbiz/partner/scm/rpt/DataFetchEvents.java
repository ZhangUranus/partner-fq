...
//         if(true)return "success";
	...
	public static String getPurchaseMatchReportSql(HttpServletRequest request) throws Exception {
		...
		String sql =" SELECT "+
						...
						" ROUND(IFNULL(CPP.BILL_ENTRY_SUM,0),4) AS BILL_ENTRY_SUM, "+
						" ROUND(IFNULL(CPP.BILL_VOLUME,0),4) AS BILL_VOLUME, "+
	...
	public static String getPackingMaterialReportSql(HttpServletRequest request) throws Exception {
		...
		String sql =" SELECT " +
						...
						" ROUND(SUM(ROUND(IFNULL(PIE.ENTRYSUM,0),4))/SUM(ROUND(IFNULL(PIE.VOLUME,0),4)),4) AS PRICE, " +
	...
	public static String queryProductionPlan(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String sql =" SELECT " +
					...
					" LEFT JOIN UNIT UNT ON TM.DEFAULT_UNIT_ID = UNT.ID " +
					" LEFT JOIN (SELECT * FROM CUR_MATERIAL_BALANCE WHERE VOLUME !=0) CMB ON MBE.ENTRY_MATERIAL_ID = CMB.MATERIAL_ID " +
					" LEFT JOIN WAREHOUSE WH ON CMB.WAREHOUSE_ID = WH.ID " +
					" WHERE MB.ID = '" + bomId + "'";
		CommonEvents.writeJsonDataToExt(response, executeSelectSQL(request,sql));
		return "sucess";
	...
	public static String getProductReportSql(HttpServletRequest request) throws Exception {
		...
		String sql =" SELECT "+
						...
						" ROUND(SUM(ROUND(IFNULL(CMB.BEGINSUM,0),4))/SUM(ROUND(IFNULL(CMB.BEGINVOLUME,0)*IFNULL(PM.BOARD_COUNT,0),4)),4) AS BEGINPRICE, "+
						" SUM(ROUND(IFNULL(CMB.VOLUME,0)*IFNULL(PM.BOARD_COUNT,0),4)) AS ENDVOLUME, "+
						" SUM(ROUND(IFNULL(CMB.TOTAL_SUM,0),4)) AS ENDSUM, "+
						" ROUND(SUM(ROUND(IFNULL(CMB.TOTAL_SUM,0),4))/SUM(ROUND(IFNULL(CMB.VOLUME,0)*IFNULL(PM.BOARD_COUNT,0),4)),4) AS ENDPRICE, "+
	...
	public static String getProductOutReportSql(HttpServletRequest request) throws Exception {
		...
		if(org.ofbiz.partner.scm.pricemgr.Utils.isCurPeriod(cal.getTime())){
			sql =" SELECT  " +
					 ...
					 " SUM(ROUND(IFNULL(CMB.VOLUME,0)*IFNULL(PM.BOARD_COUNT,0),4)) AS ENDVOLUME, "+
					 " SUM(ROUND(IFNULL(CMB.IN_VOLUME,0)*IFNULL(PM.BOARD_COUNT,0),4)) AS INVOLUME, "+
					 " SUM(ROUND(IFNULL(IFNULL(CMB.OUT_VOLUME,0)+IFNULL(POET.VOLUME,0),0)*IFNULL(PM.BOARD_COUNT,0),4)) AS OUTVOLUME "+
...
//					" GROUP BY TM.NAME,DATE(PO.BIZ_DATE),POE.GOOD_NUMBER ";
		...
		String sql =" SELECT   \r\n"+ 
					...
					" SELECT  \r\n"+ 
					" 	TM.NAME, \r\n"+ 
					" 	DATE(PR.BIZ_DATE) AS BIZ_DATE, \r\n"+ 
					" 	PR.NUMBER AS GOOD_NUMBER, \r\n"+ 
					" 	SUM(-PM.BOARD_COUNT) AS VOLUME \r\n"+ 
					" FROM PRODUCT_RETURN PR  \r\n"+ 
					" LEFT JOIN PRODUCT_RETURN_ENTRY PRE ON PR.ID = PRE.PARENT_ID \r\n"+ 
					" LEFT JOIN PRODUCT_MAP PM ON PRE.MATERIAL_MATERIAL_ID = PM.MATERIAL_ID \r\n"+ 
					" LEFT JOIN T_MATERIAL TM ON PM.ENTRY_MATERIAL_ID = TM.ID   \r\n"+ 
					" WHERE PM.ENTRY_MATERIAL_ID ='"+material_id+"'  \r\n"+ 
					" 	AND YEAR(PR.BIZ_DATE) = "+year+"  \r\n"+ 
					" 	AND MONTH(PR.BIZ_DATE) ="+month+"   \r\n"+ 
					" 	AND PR.STATUS = 4 \r\n"+ 
					" GROUP BY TM.NAME, \r\n"+ 
					" 	DATE(PR.BIZ_DATE), \r\n"+ 
					" 	PR.NUMBER \r\n";
	...
	public static String queryMaterialVolumeDetail(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String sql =" SELECT " +
					...
					" FROM (SELECT MATERIAL_ID,SUM(IFNULL(VOLUME,0)) AS VOLUME,SUM(IFNULL(TOTAL_SUM,0)) AS TOTAL_SUM FROM CUR_MATERIAL_BALANCE GROUP BY MATERIAL_ID) CMB " +
					" LEFT JOIN T_MATERIAL TM ON CMB.MATERIAL_ID = TM.ID " +
					" LEFT JOIN (SELECT MATERIAL_ID,SUM(IFNULL(VOLUME,0)) AS VOLUME,SUM(IFNULL(TOTALSUM,0)) AS TOTALSUM FROM CUR_WORKSHOP_PRICE GROUP BY MATERIAL_ID) CWP ON CMB.MATERIAL_ID = CWP.MATERIAL_ID " +
					" LEFT JOIN (SELECT MATERIAL_ID,SUM(IFNULL(VOLUME,0)) AS VOLUME,SUM(IFNULL(TOTALSUM,0)) AS TOTALSUM FROM CUR_CONSIGN_PRICE GROUP BY MATERIAL_ID) CCP ON CMB.MATERIAL_ID = CCP.MATERIAL_ID " +
					materialString +
