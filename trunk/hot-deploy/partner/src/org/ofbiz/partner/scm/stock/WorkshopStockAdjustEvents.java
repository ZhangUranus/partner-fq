 ...
 * 
 */
public class WorkshopStockAdjustEvents {
	private static final String module = org.ofbiz.partner.scm.stock.WorkshopStockAdjustEvents.class.getName();
	...
	public static String submitBill(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (delegator != null && billId != null) {
				Debug.log("车间调整单提交:" + billId, module);
				GenericValue billHead = delegator.findOne("WorkshopStockAdjust", UtilMisc.toMap("id", billId), false);
				if (billHead == null || billHead.get("bizDate") == null) {
					throw new Exception("can`t find WorkshopStockAdjust bill or bizdate is null");
				}
				if(billHead.getString("status").equals("4")){
					throw new Exception("单据已提交，请刷新数据！");
				}

				BizStockImpFactory.getBizStockImp(BillType.WorkshopStockAdjust).updateStock(billHead, false, false);
	...
	public static String rollbackBill(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			Delegator delegator = (Delegator) request.getAttribute("delegator");
			if (delegator != null && billId != null) {
				Debug.log("车间调整单撤销:" + billId, module);
				GenericValue billHead = delegator.findOne("WorkshopStockAdjust", UtilMisc.toMap("id", billId), false);
				if (billHead == null || billHead.get("bizDate") == null) {
					throw new Exception("can`t find WorkshopStockAdjust bill or bizdate is null");
				}
				...
				BizStockImpFactory.getBizStockImp(BillType.WorkshopStockAdjust).updateStock(billHead, true, true);
