...
public class WorkshopDrawMaterialBizImp implements IBizStock {
	...
	public synchronized void updateStock(GenericValue billValue, boolean isOut, boolean isCancel) throws Exception {
		...
		// 获取单据id分录条目
		List<GenericValue> entryList = delegator.findByAnd("WorkshopDrawMaterialEntry", UtilMisc.toMap("parentId", billValue.getString("id")));
		...
		for (GenericValue v : entryList) {
			...
			if(volume.compareTo(BigDecimal.ZERO)<=0){
				throw new Exception("制造领料数量不能小于等于零，请重新输入！");
			}
			BigDecimal sum = null;
			if(isOut){
