	...
	public void updateStock(GenericValue billValue, boolean isOut, boolean isCancel) throws Exception {
		...
		for (GenericValue v : entryList) {
			...
			if(volume.compareTo(BigDecimal.ZERO)<0){
				throw new Exception("委外入库数量不能为零，请重新输入！");
			}
			...
			// 构建计算条目
			PriceCalItem item = new PriceCalItem(bizDate, warehouseId, materialId, volume, sum, BillType.PurchaseWarehouse, v.getString("id"), isOut, isCancel, null);
