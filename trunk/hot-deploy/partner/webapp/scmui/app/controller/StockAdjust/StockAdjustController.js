Ext.define('SCM.controller.StockAdjust.StockAdjustController', {
			extend : 'Ext.app.Controller',
			mixins : ['SCM.extend.exporter.Exporter', 'SCM.extend.controller.BillCommonController'],
			views : ['StockAdjust.ListUI', 'StockAdjust.EditUI'],
			stores : ['StockAdjust.StockAdjustStore', 'StockAdjust.StockAdjustEditStore', 'StockAdjust.StockAdjustEditEntryStore'],
			requires : ['SCM.model.StockAdjust.StockAdjustActionModel'],
			gridTitle : '仓库调整单',
			editName : 'stockadjustedit',
			editStoreName : 'StockAdjustEditStore',
			entityName : 'StockAdjust',
			modelName : 'StockAdjustEditModel',
			entryModelName : 'StockAdjustEditEntryModel',
			actionModelName : 'StockAdjustActionModel',
			init : function() {
				this.control({
							'stockadjustlist' : {
							...
							// 列表新增按钮
							'stockadjustlist button[action=addNew]' : {
								click : this.addNewRecord
							},
							// 列表事件
							'stockadjustlist gridpanel[region=center]' : {
								select : this.showDetail,
								itemdblclick : this.modifyRecord
							},
							// 列表修改按钮
							'stockadjustlist button[action=modify]' : {
								click : this.editRecord
							},
							// 列表删除按钮
							'stockadjustlist button[action=delete]' : {
								click : this.deleteRecord
							},
							// 列表界面刷新
							'stockadjustlist button[action=search]' : {
								click : this.refreshRecord
							},
							// 列表提交按钮
							'stockadjustlist button[action=submit]' : {
								click : this.submitBill
							},
							// 列表撤销按钮
							'stockadjustlist button[action=rollback]' : {
								click : this.rollbackBill
							},
							// 列表打印按钮
							'stockadjustlist button[action=print]' : {
								click : this.print
							},
							// 列表导出
							'stockadjustlist button[action=export]' : {
								click : this.exportExcel
							},
							// 编辑界面分录新增
							'stockadjustedit  gridpanel button[action=addLine]' : {
								click : this.addLine
							},
							// 编辑界面分录删除
							'stockadjustedit  gridpanel button[action=deleteLine]' : {
								click : this.deleteLine
							},

							// 编辑界面直接提交
							'stockadjustedit button[action=submit]' : {
								click : this.saveAndSubmitRecord
							},
							// 编辑界面保存
							'stockadjustedit button[action=save]' : {
								click : this.saveRecord
							},
							// 编辑界面打印
							'stockadjustedit button[action=print]' : {
								click : this.print
							},
							// 编辑界面重填
							'stockadjustedit button[action=clear]' : {
								click : this.clear
							},
							// 编辑界面取消
							'stockadjustedit button[action=cancel]' : {
								click : this.cancel
							},
							// 监听各field值变动事件，只监听可见控件
							'stockadjustedit form textfield{isVisible()}' : {
								change : this.fieldChange
							},
							// 角色列表更新事件
							'stockadjustedit grid' : {
...
//				this.totalFields = this.editForm.down('textfield[name=totalsum]');
			...
			refreshRecord : function() {
				...
				if (!Ext.isEmpty(this.searchStartDate.getValue())) {
					tempString += 'StockAdjustV.biz_date >= \'' + this.searchStartDate.getRawValue() + ' 00:00:00\'';
				}
				if (!Ext.isEmpty(this.searchEndDate.getValue())) {
					...
					tempString += 'StockAdjustV.biz_date <= \'' + this.searchEndDate.getRawValue() + ' 23:59:59\'';
				}
//				if (this.searchMaterialId.getValue() && this.searchMaterialId.getValue() != '') {
//					if (tempString != '') {
//						tempString += ' and ';
//					}
//					tempString += 'StockAdjustEntryV.material_material_id = \'' + this.searchMaterialId.getValue() + '\'';
//				}
				...
				if ((!Ext.isEmpty(this.searchStatus.getValue())) || this.searchStatus.getValue() == 0) {
					...
					tempString += 'StockAdjustV.status = \'' + this.searchStatus.getValue() + '\'';
				}
...
//				this.totalFields.setValue(sum);
			...
			getSubmitBillUrl : function(){
				return '../../scm/control/submitStockAdjust';
			},
			...
			getRollbackBillUrl : function(){
				return '../../scm/control/rollbackStockAdjust';
			},
