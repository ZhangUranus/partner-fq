Ext.define('SCM.controller.Supplier.SupplierController', {
     ...
     * 只初始化一次，关闭时候隐藏
     */
    initSupplieredit: function(){
        if(!this.win){
            this.win = Ext.widget('Supplieredit');
        }
    },
    ...
    addNewRecord: function(button){
    	newRecord=Ext.create('SupplierEditModel');//新增记录
    ...
    saveRecord: function(button){
		...
		if(this.win.uiStatus=='Modify'){//修改记录
		    ...
		    record=Ext.create('SupplierEditModel');
		    record.set(values);
    ...
    getParams: function(){
    	...
    	with(this.listPanel.store){
	    	var params  = {
	    		...
	    		//页面参数
	    		entity: 'Supplier',				//导出实体名称，一般为视图名称。
		        title : 'Supplier',			//sheet页名称
		        header : header,			//表头
