Ext.define('SCM.controller.WorkshopWarehousing.WorkshopWarehousingController', {
	...
	afterInitComponent : function() {
		...
		this.detailEditEntry.addListener('edit', this.detailEntryEditAction, this); // 监控列表编辑事件
	...
	detailEntryEditAction : function(editor, e) {
		if (e.field == 'materialId') {
			...
			if (record) {
				...
				e.record.set('materialUnitId', record.get('defaultUnitId'));
			}
		}
	},
	...
	addDetailLine : function(button) {
		var detailRecord = Ext.create('WorkshopWarehousingDetailModel');
		detailRecord.phantom = true;
		// 设置父id
		detailRecord.set('parentId', this.currentDetailRecord.get('id'));
		detailRecord.set('bomId', this.currentDetailRecord.get('bomId'));
		// 默认单价，金额为零
		detailRecord.set('volume', 0);
		detailRecord.set('price', 0);
		detailRecord.set('entrysum', 0);
		// 设置分录id
	...
	getMainPrintHTML : function() {
		return "<div>" + "<div class='caption' >江门市蓬江区富桥旅游用品厂有限公司</div>" + "<div class='caption' >车间半成品入库单</div>"
				...
				+ "<div class='field' align='right' style='width:35%;float:right;'>日期:<span class='dataField' fieldindex='data.bizDate' width=150px></span></div>" + "<div class='nextLine'></div>"
