...
public class WorkshopReturnMaterialBizImp implements IBizStock {
	...
	public void updateStock(GenericValue billValue, boolean isOut, boolean isCancel) throws Exception {
		...
		// 获取单据id分录条目
		List<GenericValue> entryList = delegator.findByAnd("WorkshopReturnMaterialEntry", UtilMisc.toMap("parentId", billValue.getString("id")));
		...
		for (GenericValue v : entryList) {
			...
			if(volume.compareTo(BigDecimal.ZERO)<=0){
				throw new Exception("制造退料数量不能小于等于零，请重新输入！");
			}
			BigDecimal sum = null;
			if (!isOut) {
			...
			Debug.log("制造退料单价计算:物料id" + materialId + ";数量" + volume + ";金额" + sum, "WorkshopReturnMaterialBizImp");

			// 构建计算条目
			PriceCalItem item = new PriceCalItem(bizDate, warehouseId, materialId, volume, sum, BillType.WorkshopReturnMaterial, v.getString("id"), isOut, isCancel, null);

			// 计算分录单价
			PriceMgr.getInstance().calPrice(item);

			// 更新加工商库存表
			WorkshopPriceMgr.getInstance().update(workshopId, materialId, volume.negate(), sum.negate());
